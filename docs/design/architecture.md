...
默认看板地址 `127.0.0.1:3847`。绑定非回环地址时须设置 `ui_token`，或声明前面已有反向代理（`--behind-proxy`）。每个看板地址一把 ingest Bearer token；本机回环默认不强制 UI 登录。采集端填入看板 URL 后申请接入，看板设置页对照确认码批准，agent 轮询领取 token（dash 不主动连接宿主机、不下发配置）。采集端 daemon 另开本机面板（默认 `127.0.0.1:3848`），只绑回环，是配置主入口：看/改本机配置、增删看板地址（新地址全量同步，其后按地址增量）、管理 Cursor 采集账号（自动扫描展示、手动加入，采集严格用加入时那份凭证）。两档上报间隔按 Unix 钟面刻度对齐（5m → `:00/:05`，30m → `:00/:30`），不是上次同步后再等一段；启动后立刻同步一次；上报失败按退避重试、不推进钟面刻度。
